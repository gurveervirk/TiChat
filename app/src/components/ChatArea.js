  ...
  return (
    <div className="d-flex flex-column h-100 px-4 mx-auto" style={{ width: '50em', overflow: 'auto' }}>
      <div className="tr-container">
        <div>
        <label htmlFor="file-upload" className="btn btn-outline-danger" data-bs-toggle="tooltip" title="Upload documents">
      ...
      {messages.length === 0 ? (
          <div className="text-center">
            <p className="fs-2 fw-bold text-danger"><img className='img-fluid circular-image mb-2' src="TiChat.png" alt="Logo" /><br/>{greeting} How can I help you?</p>
          </div>
        ) : (
      ...
      {messages.length === 0 && <div className="prompts d-flex flex-wrap justify-content-center">
        {prompts.map((prompt, index) => (
          <div className='slide-in-down mb-2 mx-1'>
            <button className="btn btn-outline-danger w-100 px-2 text-left" onClick={() => {
              setInputText(prompt);
        ...
        <div className="input-group shadow-sm rounded-pill">
          <div className="input-group-text" data-bs-toggle="tooltip" title="Enable Index Querying">
            <input
              ...
              style={{ backgroundColor: useQueryEngine ? 'rgb(180, 0, 0)' : '', borderColor: useQueryEngine ? 'red' : 'rgb(255, 133, 133)', boxShadow: 'none' }} // Change background color when checked
            />
          </div>
          <textarea
            value={inputText}
          ...
          <button
            type="submit"
            className="btn btn-danger border-start-0 px-3 border-end-0"
